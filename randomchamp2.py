...
rrmain = True
while rrmain:
    role = int(input("Role? (1 - Top, 2 - Jungle, 3 - Mid, 4 - ADC, 5 - Support, 6 - Kevin): "))
    rrspice = True
    while rrspice:
        spice = int(input("Spice level? (1 - None, 2 - Low, 3 - SPICE, 4 - Random Spice, 5 - Any Champion): "))
        rrchamp = True
        while rrchamp:
            length = len((lol[role-1][spice-1]))
            a = random.randint(0, int(length-1))
            print("You are playing: " + lol[role-1][spice-1][a])
            rrmain = False
            rrspice = False
            rrchamp = False
            rer = input("Do you want to reroll? [y/n]: ")
            if rer == "y":
                rr = int(input("Start over at 1 - Role, 2 - Spice, 3 - Just a new champion: ").lower())
                if rr == 1:
                    rrmain = True
                elif rr == 2:
                    rrspice = True
